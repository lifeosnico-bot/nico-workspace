#!/usr/bin/env python3
"""Nighttime Report heartbeat — writes to Obsidian + sends to Telegram.
...
def pick_top_tasks(status_text: str, limit: int = 3) -> list[str]:
    ...
    for line in sec.splitlines():
        if "|" not in line or "| P" not in line:
            continue
...
def pick_blockers(status_text: str, limit: int = 3) -> list[str]:
    # Use Vincent's Actions as blocking items for now
...
def main():
    ...
    blockers = pick_blockers(status_text, 3)

    parts = [f"Nighttime Report ({day} {date_str})"]
    parts.append("Overnight plan: no autonomous jobs yet (heartbeat only).")

    if top_tasks:
        parts.append("Focus next:")
    ...
    parts.append("Morning deliverable: Morning Report (Telegram + Obsidian).")

    msg = "\n".join(parts).strip()[:MAX_CHARS]

    write_obsidian_note(
        date_str=date_str,
        title=f"Nighttime Report — {date_str}",
        body_text=msg,
        filename=f"{date_str}_nighttime-report.md",
    )
    update_home_links()
